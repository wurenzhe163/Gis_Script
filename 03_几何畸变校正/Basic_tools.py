...
def Geemap_export(fileDirname, collection=False, image=False,rename_image=True, 
    ...
    if collection:
        ...
        if os.path.exists(fileDirname):
            ...
            geemap.ee_export_vector(vector, fileDirname, selectors=None, verbose=True, keep_zip=keep_zip, timeout=300,proxies=None)
...
def reload_variable(region,
    ...
    geeup.upload(user=eeUser,source_path=save_dir,pyramiding="MEAN",mask=False,nodata_value=None,
                 metadata_path=meta_csv,destination_path=dest,overwrite=overwrite)  #!geeup upload --source save_dir --dest dest -m meta_csv --nodata 0 -u eeUser
